...
func main() {
	...
	library.books["Let's go"] = BookEntry{
		total:  200,
		lended: 0,
	}
	library.books["Let's go further"] = BookEntry{
		total:  100,
		lended: 0,
	}
	library.books["The Ultimate Go Book & Beyond"] = BookEntry{
		total:  1,
		lended: 0,
	}
	...
	library.members["Najib"] = Member{"Najib", make(map[Title]LendAudit)}
	...
	checkOut := checkoutBook(&library, "Let's go", &member)
	fmt.Println("\nCheck out a book:")
	if checkOut {
		printLibraryBooks(&library)
		printMemberAudits(&library)
	...
	returned := returnBook(&library, "Let's go", &member)
	fmt.Println("\nCheck in a book:")
	if returned {
		printLibraryBooks(&library)
		printMemberAudits(&library)
